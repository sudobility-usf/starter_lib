...
This is the primary hook consumed by UI layers (starter_app, starter_app_rn).
...
```
starter_app / starter_app_rn
    ↓ uses
@sudobility/starter_lib (this package)
    ↓ uses
@sudobility/starter_client (API hooks)
    ↓ uses
@sudobility/starter_types (type definitions)
```

## Related Projects

- **starter_types** — Shared type definitions; imported transitively via starter_client
- **starter_client** — API client SDK; this library wraps its hooks with business logic and Zustand state
- **starter_app** — Web frontend that consumes `useHistoriesManager` from this library
- **starter_app_rn** — React Native app that consumes `useHistoriesManager` via file: links
- **starter_api** — Backend server; this library communicates with it indirectly through starter_client

## Coding Patterns

- `useHistoriesManager` is the primary hook -- it orchestrates starter_client hooks + Zustand store into a single unified interface for UI layers
